...
static MachineBasicBlock *lowerWaveReduce(MachineInstr &MI,
  ...
  if (isSGPR) {
    ...
    // unsigned MovOpcForIdentityVal =
    // is32BitOpc ? AMDGPU::S_MOV_B32 : AMDGPU::S_MOV_B32;
    unsigned ExecReg = IsWave32 ? AMDGPU::EXEC_LO : AMDGPU::EXEC;

    // Create initial values of induction variable from Exec, Accumulator and
    // insert branch instr to newly created ComputeBlock
    auto IdentityValue = getIdentityValueForWaveReduction(Opc);
    ...
    if (is32BitOpc) {
      ...
      auto IdenHi = BuildMI(BB, I, DL, TII->get(AMDGPU::S_MOV_B32), Identityhi)
                        .addImm(IdentityValue);
      switch (Opc) {
      case AMDGPU::V_CMP_LT_U64_e64:
      case AMDGPU::V_CMP_LT_I64_e64:
        IdentityValue = int32_t(-1);
        break; // min
      case AMDGPU::V_CMP_GT_U64_e64:
      case AMDGPU::V_CMP_GT_I64_e64:
        IdentityValue = int32_t(0);
        break; // max
      }
      auto IdenLo = BuildMI(BB, I, DL, TII->get(AMDGPU::S_MOV_B32), Identitylo)
                        .addImm(IdentityValue);
